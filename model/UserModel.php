...
class UserModel {
   ...
   public function actualizarCuentaValidada($email){
    ...
    public function getRanking(){
       return $this->database->query( "select ROW_NUMBER() OVER(ORDER BY SUM(puntaje) DESC) AS 'Puesto', u.nickName,u.imagenPerfil ,sum(puntaje) as 'Puntaje' from partida p join usuario u on u.id = p.idUsuario group by p.idUsuario ORDER BY SUM(puntaje) DESC LIMIT 5");
    }
}